...
public:
	...
	Frame(EtherAddress ra, EtherAddress ta, uint64_t tsft, uint16_t flags, uint16_t seq,
			int8_t rssi, uint8_t rate, uint8_t type, uint8_t subtype, uint32_t length, uint8_t retry) :
			_ra(ra), _ta(ta), _tsft(tsft), _flags(flags), _seq(seq), _rssi(rssi),
			_rate(rate), _type(type), _subtype(subtype), _length(length), _retry(retry) {
	}
};
